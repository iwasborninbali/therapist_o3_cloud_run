#!/usr/bin/env python3
"""
Universal proactive message scheduler for Telegram bot with HTTP health check server.
...
from bot.firestore_client import (  # noqa: E402
...
from bot.openai_client import get_response  # noqa: E402
from config import Config, DEFAULT_SYSTEM_PROMPT, MORNING_HOUR, EVENING_HOUR, PROACTIVE_CHECK_INTERVAL  # noqa: E402
...
def process_single_user(user_id: str, timezone_str: str) -> bool:
    """
    Process a single user for proactive messages.
...
def process_all_users():
    """
    Process all users and send proactive messages if needed (parallel processing).
    """
    try:
        ...
        logger.info(f"Processing {len(users)} users for proactive messages (parallel)")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_user):
                user_id, timezone_str = future_to_user[future]
                try:
...
def main():
    """Main function - starts both scheduler and health server"""
